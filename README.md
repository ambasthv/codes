...
margin = Inches(1)
table_total_width = slide_width - 2 * margin  # 11.33 inches for Table1 and Table2
table3_total_width = Inches(6)  # Fixed total width for Table3 as per previous specs
...
def copy_table_to_ppt(table_name, slide, left, top, total_width, header_height=Inches(0.5), data_row_height=Inches(0.4), header_rgb=(0, 44, 113), row_rgb=(231, 232, 235)):
    # Get table range
    ...
    max_lens = [0 for _ in range(cols)]
    ...
    # Dynamically calculate row heights (fixed header, equal data rows)
    row_heights = [header_height] + [data_row_height] * (rows - 1) if rows > 1 else [header_height]
...
# Step 6: Copy and paste tables to PowerPoint with dynamic layouts
# Table 1: 1 inch from top, left margin
table1, table1_height = copy_table_to_ppt("Table1", slide, margin, Inches(1), table_total_width)

# Table 2: 0.5 inch below Table 1
table2_top = Inches(1) + table1_height + Inches(0.5)
table2, table2_height = copy_table_to_ppt("Table2", slide, margin, table2_top, table_total_width)

# Table 3: 0.5 inch below Table 2, right down corner (1 inch from right)
table3_top = table2_top + table2_height + Inches(0.5)
table3, table3_height = copy_table_to_ppt("Table3", slide, Inches(0), table3_top, table3_total_width)  # Temporary left
table3_shape = table3._parent
table3_shape.left = slide_width - margin - table3_total_width

# Step 7: Copy "BreachChart1" from Excel and paste as a picture in PowerPoint
...
# Calculate chart position: 0.5 inch below Table2, 0.5 inch left to Table3 (right edge of chart aligns 0.5 inch left of Table3 left)
chart_top = table2_top + table2_height + Inches(0.5)
chart_left = table3_shape.left - Inches(0.5) - chart_width

# Add the picture at the calculated position
