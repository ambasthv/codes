...
def copy_table_to_ppt(table_name, slide, left, top, total_width, header_height=Inches(0.5), total_height=None):
    ...
    for j in range(start_col, end_col + 1):
        ...
        for i in range(start_row, end_row + 1):
            ...
            # Handle multi-line text by counting max length of any line
    ...
    total_width_inches = total_width / 914400  # Convert to inches (1 inch = 914400 EMUs)
    col_widths = [Inches(max_len / total_chars * total_width_inches) for max_len in max_lens]

    # Dynamically calculate row heights to fit within available space
    ...
    for i in range(start_row, end_row + 1):
        for j in range(start_col, end_col + 1):
            ...
            ppt_cell.text = cell_value  # Preserve multi-line text

            # Apply formatting to all lines in the cell
...
# Calculate available height to ensure everything fits
available_height = SLIDE_HEIGHT - START_TOP - Inches(0.5)  # Reserve 0.5 inch for chart bottom margin
table_heights = [Inches(1.25), Inches(3), Inches(2.5)]  # Desired heights for Table1, Table2, Table3
total_content_height = sum(table_heights) + 2 * Inches(0.5)  # Include 0.5-inch gaps between tables
chart_height_estimate = Inches(2)  # Approximate chart height
if total_content_height + chart_height_estimate > available_height:
    # Scale table heights proportionally to fit
...
# Table 1: 0.5 inch from top, left margin, dynamic column widths
table1, table1_height = copy_table_to_ppt("Table1", slide, MARGIN, START_TOP, TABLE1_2_WIDTH, header_height=Inches(0.5), total_height=table_heights[0])

# Table 2: 0.5 inch below Table 1, dynamic column/row fitting
table2_top = START_TOP + table1_height + Inches(0.5)
table2, table2_height = copy_table_to_ppt("Table2", slide, MARGIN, table2_top, TABLE1_2_WIDTH, header_height=Inches(0.5), total_height=table_heights[1])

# Table 3: 0.5 inch below Table 2, right-down corner (1 inch from right)
table3_top = table2_top + table2_height + Inches(0.5)
table3, table3_height = copy_table_to_ppt("Table3", slide, SLIDE_WIDTH - MARGIN - TABLE3_WIDTH, table3_top, TABLE3_WIDTH, header_height=Inches(0.5), total_height=table_heights[2])
