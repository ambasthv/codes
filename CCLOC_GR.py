...
# ====================== APP SETUP ======================
st.set_page_config(page_title="Model Monitoring Report", layout="wide")   # Wide layout for better table view
...
if uploaded_file is not None:
    ...
    # Auto-detect quarter name (e.g. 2025Q3) from any column header
    ...
    if quarter_col is None:
        st.error("Could not detect 2025Q3 column.")
        st.stop()
    ...
    # ====================== STANDARDIZE METRICS ======================
    # Convert raw metrics into clean keys we use in the app
    data_dict = {}
    for k, v in raw_dict.items():
        ...
        if "gini" in k_clean:           data_dict["gini"] = float(v) if pd.notna(v) else 0.0
            ...
            data_dict["balance/exposure"] = str(v).strip()   # Keep exact text from CSV (including $ if any)

    # ====================== TABLE 1: MODEL ATTRIBUTE ======================
    st.subheader("Model Attribute")
    
    # Create Model Attribute table (plain table like your original ChatGPT code)
    table1 = pd.DataFrame({
    ...
    st.table(table1)   # Plain table - no extra styling (removes green tint)
    ...
    # Create smaller breach summary chart
    ...
    # ====================== KEY OBSERVATIONS ======================
    # Extract breach details for summary text
...
"""
    ...
    st.success(f"✅ {original_quarter_name} Analysis completed successfully!")